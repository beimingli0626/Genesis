...
class PursuitEnv:
    ...
    def step(self, actions):
        ...
        self.target_actions = torch.clip(
            target_actions, -self.env_cfg["clip_target_actions"], self.env_cfg["clip_target_actions"]
        )
        ...
        self.agent_actions = torch.clip(
            actions, -self.env_cfg["clip_agent_actions"], self.env_cfg["clip_agent_actions"]
        )
        ...
        self.finish_condition = (
            ...
            < self.env_cfg["at_target_threshold"]  # distance in xy within threshold = capture
        )
    ...
    def _reward_distance(self):
        ...
        """
        return torch.norm(self.rel_pos[:, :2], dim=1)
